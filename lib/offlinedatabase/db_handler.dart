...
class DBHelper {
  ...
  _onCreate(Database db, int version) async {
    await db
        .execute('CREATE TABLE eventvisitor (id INTEGER PRIMARY KEY,name TEXT,mobileNumber TEXT,company TEXT,category TEXT,email TEXT,type TEXT)');
  }
  ...
  Future<List<Visitorclass>> getVisitors() async {
    var dbClient = await db;
    List<Map> maps = await dbClient.query('eventvisitor', columns: ['id', 'name','mobileNumber','company','category','email','type']);
